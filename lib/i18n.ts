...
const ko = {
  ...
  exercisePickerPlaceholder: '운동 기구 선택',
  exerciseSearchPlaceholder: '검색하거나 새 기구 추가',
  addExerciseNamed: (name: string) => `"${name}" 추가`,
  noExerciseMatch: '검색 결과가 없어요',
  deleteExerciseTitle: '기구 삭제',
  deleteExerciseMessage: (name: string) => `"${name}"를 목록에서 삭제할까요? (기록은 남아요)`,
  weightLabel: '중량',
  repsLabel: '횟수',
...
const en: typeof ko = {
  ...
  exercisePickerPlaceholder: 'Select an exercise',
  exerciseSearchPlaceholder: 'Search or add new',
  addExerciseNamed: (name) => `Add "${name}"`,
  noExerciseMatch: 'No matches',
  deleteExerciseTitle: 'Delete exercise',
  deleteExerciseMessage: (name) => `Remove "${name}" from the list? (Logs are kept)`,
  weightLabel: 'Weight',
  repsLabel: 'Reps',
